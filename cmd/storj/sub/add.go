    ...
    Run: func(cmd *cobra.Command, args []string) {
        ...
        if secret == "" && helpers.AppConfig.STORJ_SECRET == "" {
            helpers.HandleResponseError(
                "An error occurred at 'github.com/dalecosta1/sinaloa-cli/cmd/storj/sub/add' inside method 'AddStorjCmd': Required flag not set: secret",
                "500",
                nil,
            )
            return
        ...
        if err != nil {
            helpers.HandleResponseError(
                "An error occurred at 'github.com/dalecosta1/sinaloa-cli/cmd/storj/sub/add' inside method 'AddStorjCmd' calling the function be 'be.NewStorjService(secret)'",
                "401", 
                err,
            )
            return
        ...
        if err != nil {
            helpers.HandleResponseError(
                "An error occurred at 'github.com/dalecosta1/sinaloa-cli/cmd/storj/sub/add' inside method 'AddStorjCmd' calling the function be 'uplink.storjService.AddFile'",
                "500",
                err,
            )
            return
        ...
        if err != nil {
            helpers.HandleResponseError(
                "An error occurred at 'github.com/dalecosta1/sinaloa-cli/cmd/storj/sub/add' inside method 'AddStorjCmd' while marshaling JSON",
                "500",
                err,
            )
            return
